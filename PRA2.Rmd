...
packages <- c("ggplot2", "plyr", "dplyr", "stats", "lmtest", "tidyverse", 
              "caret", "corrplot", "ggpubr", "Hmisc", "PerformanceAnalytics", 
              "factoextra", "rpart", "rpart.plot", "factoextra", "useful", 
...
# Nota: revisant el dataset, veig que potser podríem fer un promig per províncies per evitar duplicats en les dades provinents del dataset ESIOS
...
equival a $\pm 3\sigma$ respecte el valor promig.

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Identificació de valors extrems de les variables numèriques
final_dataset %>%
  select(-date) %>%
  mutate_if(is.numeric, ~ ifelse(abs(.) > 3 * sd(.), NA, .)) %>%
