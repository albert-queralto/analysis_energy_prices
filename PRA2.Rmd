...
equival a $\pm 3\sigma$ respecte el valor promig. Per això, podem normalitzar
...
# Subset del dataset amb les columnes numèriques i sense la data
subset_final <- final_dataset %>% select(num_vars)

# Eliminem la columna de la data
subset_final <- subset_final[,-1]

# # Estandarditzem les columnes numèriques entre -1 i 1
normalized_data <- scale(subset_final, center = mean_num_vars, scale = sd_num_vars)

# Convertim les columnes no atòmiques a vectors atòmics
normalized_data <- as.data.frame(lapply(normalized_data, as.vector))
head(normalized_data)
# # Boxplots de les variables estandarditzades cada columna un boxplot
# normalized_data %>% 
