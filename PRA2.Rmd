...
packages <- c("ggplot2", "plyr", "dplyr", "stats", "lmtest", "tidyverse", 
              ...
              "pROC", "zoo", "reshape2", "gridExtra", "knitr")

# Comentar si no cal instal·lar les llibreries 
# install.packages(packages, repos="https://cran.rediris.es/")
...
Es pot veure que algunes columnes contenen valors nuls tot i que aquests representen
un percentatge molt reduït si tenim en compte les dimensions del dataset (105120
files). Existeixen diversos mètodes per tractar-los, entre ells la imputació amb
el valor promig. Tot i que aquest mètode podria ser vàlid en aquest cas, en
...
les categories de la columna *energy_source* respecte les columnes 
...
A continuació, representem les variables numèriques del dataset resultant en 
diferents histogrames:
...
Observem que les variables *renewable_generation_mw* i *renewable_generation_perc*
segueixen un mateix patró (una essent el percentatge de l'altra). La frequència
es presenta més alta, al voltant de 800 MW, quan els valors disminueixen i 
decreix quan els valors incrementem, essent gairebé nul·la en valors de 30000.

D'altra banda, el *avg_total_price_eur_mwh* presenta una clara tendència a la 
baixa quan els valors de preu incrementen i una freqüència a l'alça quan els
valors de preu decreixen.

Finalment, la variable *energy_total_mwh* presenta una freqüència més elevada 
entre els valors de 25000 i 30000 MWh.
...
En la gràfica superior observem com la font d'energia més utilitzada a Espanya 
és la hidroelèctrica, seguida per la nuclear i la solar en segona i tercera posició.
En aquest cas, l'energia eòlica seria la menys utilitzada.


Eliminem la columna *renewable_generation_mw*, perquè aporta informació
redundant respecte *renewable_generation_perc*
...
S'observa un percentatge de generació entre el 0% i el 35% per totes les fonts 
d'energia. Pel que fa a l'energia eòlica, aquesta presenta un percentatge superior
en el rang del 36% al 75%.
...
Comprovem com totes les variables de les que disposa el dataset són de classe
character excepte les variables *height_above_sea* i *max_wind_direction* que són
de classe numèrica. Les variables de tipus numèric mostren diverses mètriques de
les dades, com ara la mínima, màxima, mediana o mitjana de la variable.
...
# Creem un grid amb els QQ-plots
# grid.arrange(
#   qqnorm(as.numeric(subset_energy_source_total$renewable_generation_perc)) +
#     qqline(as.numeric(subset_energy_source_total$renewable_generation_perc)) +
#     labs(title=" QQ-plot of renewable_generation_perc") +
#     theme_bw() +
#     theme(text = element_text(size = 20)),
#   qqnorm(as.numeric(subset_energy_source_total$avg_total_price_eur_mwh)) +
#     qqline(as.numeric(subset_energy_source_total$avg_total_price_eur_mwh)) +
#     labs(title=" QQ-plot of avg_total_price_eur_mwh") +
#     theme_bw() +
#     theme(text = element_text(size = 20)), ncol = 2)
```
