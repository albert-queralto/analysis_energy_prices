...
# Afegir explicació (com són les estadístiques de les dades, valors nuls)
...
### 2.1.6. Calcular percentatge d'energia renovable respecte el total
...
### 2.1.7. Representem histogrames de les variables

# Afegir gràfiques d'altres dades també separant pels grups de variables categòriques i analitzar els resultats
...
Representem histogrames de les variables *avg_total_price_eur_mwh*,
*avg_price_free_market_eur_mwh* i *avg_price_ref_market_eur_mwh*:

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Representar l'histograma per avg_total_price_eur_mwh, 
# avg_price_free_market_eur_mwh i avg_price_ref_market_eur_mwh
ggplot(esios_dataset, aes(x = avg_total_price_eur_mwh)) + 
  geom_histogram(bins = 50, color = "black", fill = "blue", alpha = 0.5) + 
  geom_histogram(aes(x = avg_price_free_market_eur_mwh), bins = 50, color = "black", 
                                                  fill = "red", alpha = 0.5) +
  geom_histogram(aes(x = avg_price_ref_market_eur_mwh), bins = 50, color = "black",
                                                  fill = "green", alpha = 0.5) +
  scale_fill_manual(name = "Tipus de mercat", 
                    values = c("blue", "red", "green"), 
                    labels = c("Total", "Mercat lliure", "Mercat referència")) +
  labs(x = "Preu mitjà (€/MWh)", 
        y = "Freqüència") + 
  theme_bw() + 
  theme(text = element_text(size = 30))
  theme(plot.width = 200, plot.height = 100)
...
Representem histogrames de les variables *energy_total_mwh*,
*energy_free_market_mwh* i *energy_ref_market_mwh*:

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Representar l'histograma per energy_total_mwh, 
# energy_free_market_mwh i energy_ref_market_mwh
ggplot(esios_dataset, aes(x = energy_total_mwh)) + 
  geom_histogram(bins = 50, color = "black", fill = "blue", alpha = 0.5) + 
  geom_histogram(aes(x = energy_free_market_mwh), bins = 50, color = "black", 
                                                  fill = "red", alpha = 0.5) +
  geom_histogram(aes(x = energy_ref_market_mwh), bins = 50, color = "black",
                                                  fill = "green", alpha = 0.5) +
  scale_fill_manual(name = "Tipus de mercat", 
                    values = c("blue", "red", "green"), 
                    labels = c("Total", "Mercat lliure", "Mercat referència")) +
  labs(x = "Generació d'energia (MW)", 
        y = "Freqüència") + 
  theme_bw() + 
  theme(text = element_text(size = 30))
  theme(plot.width = 200, plot.height = 100)
...
# Nota: la representació de les variables renewable_generation_mw i renewable_generation_perc es fa més avall després del recàlcul

### 2.1.8. Crear intervals per *renewable_generation_perc*
...
### 2.1.9. Representem l'histograma de les variables *renewable_generation_mw*, *renewable_generation_perc* i *renewable_generation_perc_bin*
...
### 2.1.10. Eliminem les columnes que no ens interessin fent servir una matriu de correlació
...
El resultat mostra que hi ha una elevada correlació entre les variables 
*renewable_generation_mw* i *renewable_generation_perc*, per tant, només ens
quedarem amb la variable *renewable_generation_perc*. Les variables
*avg_price_free_market_eur_mwh*, *avg_price_ref_market_eur_mwh* i
*avg_total_price_eur_mwh* també tenen una elevada correlació, per això només
...
s'ha decidit eliminar-les ja que s'ha considerat que no aporten
informació rellevant per a l'estudi proposat. Així doncs, les columnes que es 
volen conservar són: *date*, *avg_total_price_eur_mwh*, *energy_total_mwh*, 
*free_market_share_perc*, *ref_market_share_perc*, *energy_source* i 
...
### 2.1.11. Transformar *date*, *energy_source* i *renewable_generation_perc_bin* al format correcte
...
### 2.2.3. Calcular promitjos de les variables numèriques
...
### 2.2.4. Creació de la variable *avg_pressure*
...
### 2.2.5. Discretització de la variable *avg_pressure*
...
### 2.2.6. Discretització de la temperatura mitjana
...
# Afegir explicació del que es veu. Com és l'histograma? distribució normal, biax, etc.

Sembla que les variables *min_daily_temp* i *max_daily_temp* són variables de les
quals es podria prescindir. Per tant, podem mirar de discretitzar la variable
*avg_daily_temp* fent servir rangs de 10 graus de temperatura. Per això,
representem la distribució estadística de les dades:
  
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Distribució estadística de la temperatura mitjana
summary(aemet_dataset$avg_daily_temp)
```

Es pot veure com el valor mínim és d'uns -8 ºC, mentre que el màxim és de 36 ºC.
Per tant, podem discretitzar la variable *avg_daily_temp* en 5 intervals:
    
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Discretització de la temperatura mitjana
aemet_dataset <- aemet_dataset %>% 
  mutate(avg_daily_temp_bin = cut(avg_daily_temp, 
                                  breaks = c(-Inf, 0, 10, 20, 30, Inf), 
                                  labels = c("Below 0", "0-10", "10-20", 
                                                          "20-30", "Above 30")),
                                  include.lowest = TRUE, right = FALSE)
```

Tot seguit, representem la temperatura mitjana discretitzada:
  
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Gràfic de barres de la temperatura mitjana
ggplot(aemet_dataset, aes(x = avg_daily_temp_bin)) + 
  geom_bar(fill = "blue", color = "black", alpha = 0.5) +
  scale_x_discrete(name = "Temperatura mitjana") +
  scale_y_continuous(name = "Nombre d'observacions") +
  labs(title = "Distribució de la temperatura mitjana") +
  theme_bw() +
  theme(text = element_text(size = 30))
  theme(plot.width = 200, plot.height = 100)
```
...
### 2.2.8. Histogrames de la velocitat del vent
...
### 2.2.9. Eliminem les columnes innecessàries fent servir una matriu de correlació
...
corr_matrix_aemet <- cor(aemet_dataset[, c("height_above_sea", 
          ...
          "min_pressure", "avg_pressure")], use = "complete.obs", 
          method=c("pearson"))
...
*min_pressure*, aquestes estan altament correlacionades amb *avg_pressure*. Per
això, conservarem la darrera variable i eliminarem la resta. Finalment, també
eliminarem la direcció del vent, ja què hem considerat que no és útil per al
nostre anàlisis.
...
*insolation*, *avg_pressure* i les variables discretitzades:

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Eliminem les columnes que no ens interessin
aemet_dataset <- aemet_dataset %>% 
  select(date, province, avg_daily_temp, daily_precipitation, avg_wind_speed, 
          max_wind_speed, insolation, avg_pressure, avg_pressure_bin, 
          avg_daily_temp_bin, insolation_bin)

# Mostrem les primeres files del dataset
head(aemet_dataset)
```

### 2.2.10. Transformació de les variables al format correcte
