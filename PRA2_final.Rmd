...
Representem les variables numèriques del dataset resultant en diferents histogrames:

```{r echo=TRUE, message=FALSE, warning=FALSE, fig.height=2, fig.width=10}
...
esios_dataset %>%
  ...
  facet_wrap(~key, scales = "free", ncol = 4) + labs(x = "Valors", y = "Frequència") +
  theme_bw() + theme(text = element_text(size = 11))
```

Observem que les variables *renewable_generation_mw* i *renewable_generation_perc* segueixen un mateix patró (una essent el percentatge de l'altra). S'observa una freqüència més alta al voltant de 800 MW, mentre què aquesta disminueix a mesura que augmenta la potència renovable, essent gairebé nul·la en valors de 30000 MW. Això indicaria que la producció renovable és normalment baixa, entre el 0 i el 25%. L'*avg_total_price_eur_mwh* presenta dues distribucions, la primera amb un màxim entorn a 100 MW i la segona amb un màxim al voltant de 200 MW. Això indicaria la presència de períodes amb dos rangs de preus molt diferents. La variable *energy_total_mwh* presenta una distribució lleugerament esbiaxada cap a l'esquerra amb un màxim de freqüència entorn els 28000 MWh.

Representem la sèrie temporal de les mateixes variables:
...
total_price_plot <- ggplot(esios_dataset, aes(x = date, y = avg_total_price_eur_mwh)) + geom_line() + labs(title = "avg_total_price_eur_mwh", 
        x = "Date", y = "Avg total price eur mwh") + theme(plot.title = element_text(hjust = 0.5)) +
  theme_bw() + theme(text = element_text(size = 11))
...
renewable_generation_plot <- ggplot(esios_dataset, aes(x = date, y = renewable_generation_mw, color = energy_source)) + geom_line() + labs(title = "renewable_generation_mw",
        x = "Date", y = "Renewable generation mw") + theme(plot.title = element_text(hjust = 0.5)) +
  theme_bw() + theme(text = element_text(size = 11))
...
renewable_generation_perc_plot <- ggplot(esios_dataset, aes(x = date, y = renewable_generation_perc, color = energy_source)) + geom_line() + labs(title = "renewable_generation_perc",
        x = "Date", y = "Renewable generation perc") + theme(plot.title = element_text(hjust = 0.5)) +
  theme_bw() + theme(text = element_text(size = 11))

grid.arrange(total_price_plot, energy_total_plot, renewable_generation_plot, renewable_generation_perc_plot, ncol = 2)
```
